...
const Card = ({ priority, title, assignTo,checklist, myTaskId, serverFetchedDate, collasped }) => {
    ...
    const fetchEmailById = async (objectId) => {
        try {
            const response = await axios.get(`${baseUrl}/api/useremail`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                },
                params: {
                    objectId // Passing the objectId as a query parameter
                }
            });
            setEmail(response.data.email); // Assuming your backend returns an object with an email field
        } catch (error) {
    ...
    useEffect(() => {
        if (assignTo) {
            fetchEmailById(assignTo); // Call the function with the ObjectId
        }
    }, [assignTo]);
    ...
    const confirmDelete = async () => {
        await deleteTask(myTaskId); // Call the delete function
            ...
            <div className={StylesCard.card}>
                <div className={StylesCard.priorityText} style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div><img src={imgSrc} alt='high' />&nbsp;&nbsp;{priority}</div> 
                                    {email && (
                        <div className={StylesCard.profile}>
                            <div className={StylesCard.profileImg}>
                    ...
                    <div style={{ position: 'relative', display: 'inline-block' }}>
                        ...
                        {showOverlay && (
                            <div className={StylesCard.dropDown} style={{ display: 'flex', flexDirection: 'column', gap: '11px' }}>
                                ...
                                <div className={StylesCard.dropDownButDel} onClick={() => setIsModalVisible(true)}>Delete</div>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className={StylesCard.checklist}>
                    ...
                    <button onClick={toggleVisibility} className={`${isVisible ? StylesCard.hideBut : StylesCard.showBut}`} style={{ width: '21px', height: '21px', position: 'relative', left: '195px' }}>
                    </button>
                </div>
                ...
                {((checklist && isVisible) || (collasped === true)) && (
                    <div>
                        ...
                        {checklist.map((taskList, index) => (
                            <TaskList key={taskList._id} taskName={taskList.taskName} completed={taskList.completed} taskListId={myTaskId} checkListId={taskList._id} myChecklistDisplay={funTotalChecksMarked} />
                        ))}
                    </div>
                )}

                <br />
                <div className={StylesCard.cardFooter} style={{ display: 'flex', justifyContent: 'spaceBetween', paddingRight: '35px' }}>
                    {
                        newDueDate ?
                    ...
                    <div className={StylesCard.cardFooter} style={{ position: 'relative', right: '-21px', display: 'flex', gap: '10px' }}>
